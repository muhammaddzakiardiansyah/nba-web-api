...
const productsContrller = {
  ...
  checkout: async (req, res) => {
    try {
      ...
      const check = await productsModel.checkout(request);
      return res.status(200).send({ message: 'transaksion success!', data: request });
    } catch (error) {
      return res.status(error.status).send({ message: 'transaksion failed!', error: error });
    }
  },
